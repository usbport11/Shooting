...
bool MWindow::EnterFullscreen()
{
	...
	if(Fullscreen)
	{
		LogFile<<"Window: alredy fullscreen"<<endl;
		return false;
	} 
	...
	if(!SetWindowPos(m_hWnd, 0, 0, 0, WindowWidth, WindowHeight, SWP_NOZORDER | SWP_NOACTIVATE | SWP_DRAWFRAME)) LogFile<<"Window: SetWidowPos failed"<<endl;
	
	return true;
}

bool MWindow::ExitFullscreen()
{
	...
	if(!ChangeWindowSize(WindowWidth, WindowHeight)) return false;
